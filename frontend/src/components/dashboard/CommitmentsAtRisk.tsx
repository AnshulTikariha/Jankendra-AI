...
export function CommitmentsAtRisk({ items }: Props) {
  return (
    <section className="overflow-hidden rounded-3xl border border-rose-200/80 bg-gradient-to-b from-rose-50 to-white shadow-md shadow-rose-100/50">
      <div className="border-b border-rose-100 px-5 py-4">
        <div className="flex items-center gap-3">
          <div className="grid size-10 place-items-center rounded-2xl bg-gradient-to-br from-rose-500 to-orange-500 text-white shadow-lg shadow-rose-200">
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-rose-600">Accountability</p>
      ...
      <ul className="space-y-3 p-4">
        {items.map((item) => (
          <li className="rounded-2xl border border-rose-100 bg-white/80 p-4 shadow-sm" key={item.id}>
            <div className="flex items-center justify-between gap-2">
              <span className="rounded-full bg-rose-100 px-2.5 py-1 text-xs font-extrabold text-rose-700">
                {item.weightTier}
              </span>
              <span className="text-xs font-bold text-rose-600">{item.daysOverdue}d overdue</span>
            </div>
